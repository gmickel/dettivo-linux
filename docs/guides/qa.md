...
Every behaviour the product promises has an automated route that proves it on a real desktop. A release itself needs only the tag's CI run ([docs/RELEASING.md](../RELEASING.md)); the rig is what you reach for to prove a change in depth. This guide is the path through the rig for someone who has to prove a change or ship a build: the rig itself, the packs, the visual matrix, the benchmarks, the evidence map that ties every requirement to a route, the release gate that runs them all, and how to write a scenario. Each section links the page that holds the details ([docs/qa.md](../qa.md) is the reference).
