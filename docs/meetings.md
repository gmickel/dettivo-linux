...
**The rule.** Every remote line gets the speaker who holds most of its sentence ([ADR 0072](adr/0072-every-remote-line-takes-the-speaker-of-its-sentence.md)). The pass cuts the remote segments at sentence ends and, where the engine aligned words, at a pause of at least `pause_ms` (250) where the diarized speaker changes across it. A sentence that runs over several segments votes once, so it is never split between two speakers, and a segment holding two sentences by two speakers becomes two stored segments with their own times, text and polished text. A sentence no turn overlaps takes the nearest turn within `nearest_turn_ms` (10 s) and otherwise stays unlabelled. `min_speaker_share` (0 by default) is the share of a sentence the winner must hold, and 0.6 brings back blank lines where two voices share a sentence evenly. On the fn-67 bench the rule left 1.1% of the retained meetings' remote lines blank, down from 18.0%, and on AMI dev it gave 20.6% of named lines the wrong speaker, up from 15.4%. Microphone segments are `You` (`speaker_id = you`); in a room-audio meeting every segment is assigned from the turns. The engine's labels are renumbered in order of first appearance, so `speaker_00` (`Speaker 1`) is whoever spoke first and a re-run lands on the same ids.

**On the row.** `meetings.get` and `transcripts.get` carry `speakers` (`speaker_id`, `name`, `color_index` in order of first appearance with `you` first, `talk_ms` summed over the speaker's segments), `diarization` (`status`, `coverage` of the track, `engine`, `model`, `ran_at`, `error`, `expected_speakers`, `auto`) and, on every segment, `speaker` (the name), `speaker_id` and `speaker_confidence` (the winner's share of the sentence's diarized speech, 0 for a nearest-turn name); `meetings.list` items carry `speaker_count`. `dettivo meetings segments <id>` prints the name where one is assigned and the side where none is.
