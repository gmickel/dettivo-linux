...
A release is a version bump and a tag. You prove the change the way every change is proven, with `just build test lint` and by using it, and the tag does the rest: CI tests the tagged commit, builds the package, installs it on a clean Arch container, and publishes it to GitHub and the plugin mirror, in about 45 minutes and without touching your machine ([ADR 0070](adr/0070-a-release-is-local-qa-and-a-tag.md)). Ordinary pull requests never build a release.

## Cutting a release

1. Bump the version in `Cargo.toml`, the `pkgver` of the three recipes under `packaging/aur/` (then regenerate each `.SRCINFO` with `makepkg --printsrcinfo`), and `omarchy/manifest.json`. `scripts/packaging/lint.sh` and `scripts/lint-omarchy-plugin.sh` refuse a version that differs anywhere.
2. Turn `## [Unreleased]` in `CHANGELOG.md` into `## [<version>] - <date>` and open a fresh `## [Unreleased]` above it.
3. Merge that through a pull request like any change, then tag the merge commit:
...
`CHANGELOG.md` keeps one `## [<version>]` section per release in the Keep a Changelog shape; the workflow takes it as the release notes and fails before building when the section is missing or the tag differs from `Cargo.toml`'s version. The tag runs [`.github/workflows/release.yml`](../.github/workflows/release.yml). `prepare` decides through [scripts/packaging/release-decide.sh](../scripts/packaging/release-decide.sh): a tag push publishes, a dispatch publishes only with `dry_run=false` on that tag, and every other run rehearses whatever ref it runs on. The rig, called with `release: true`, then runs `just toolchain test lint` and `just package-bin` once and installs that package on a clean Arch container, and `publish` ships that same tarball, `SHA256SUMS`, the pacman package and the notes with `gh release create`, downloading the published files to verify their checksums. The whole run takes about 45 minutes. Dispatching the workflow by hand with `dry_run` (the default) does everything but the publication and leaves the files as workflow artifacts, which is how a release candidate is rehearsed on a branch. `scripts/packaging/test-release-scripts.sh` proves the decision in `just lint`.
...
## The optional deep gate

`dettivo-qa pack release` (`just qa-release`) is the thirteen-step gate the build used before releases were cut from a tag alone. It no longer blocks anything. Run it when you want the whole product checked on this desktop at once: every pack, the benchmark, the install test and a real-microphone receipt, filed as a report under [docs/reports/release-gate/](reports/release-gate/0.1.0.md). The rig's deep jobs (the GUI drives, the benchmark, the separate Vulkan builds and the source package) run the same way, by hand, with `gh workflow run rig.yml`.
