...
Dettivo for Linux is the native Omarchy/Hyprland port of Gordon Mickel's speech workstation: a Rust daemon, Qt 6 Quick surfaces and ggml engine processes. This file is how an agent works in this repository. `AGENTS.md` carries the same instructions for hosts that read that name.
...
- **PR review bots: Cursor Bugbot or GitHub Copilot, whichever completes a review.** Bugbot reviews every push on its own; request Copilot only when Bugbot has not reviewed the current head. A PR merges when one of them has reviewed the head it merges and no review thread is open: fix or answer each finding first. Never trigger or count the Codex bot or any other review bot; the gpt-6-astra reviews above run locally through the CLIs.
...
- **Design is the bar.** Every surface resolves from Omarchy theme tokens and is beautiful on every theme. The baselines under `docs/design/` are references, and a person judges the running product.
- **Visual checks are optional tools, never gates.** `just qa-visual` and its companions run by hand on the desktop when someone wants to look (ADR 0066). Never add them, or any render diff against the baselines, to a GitHub workflow or to the release gate.
