...
- **Visual regression runs on the desktop, never in CI.** `just qa-visual` and `just qa-visual-canary` run locally on Omarchy before a change that touches a surface, and as steps of `dettivo-qa pack release` (ADR 0066). Do not add them, or any job that diffs renders against the baselines, to a GitHub workflow: a CI container draws fonts and themes differently from the desktop the baselines were approved on, so its failures say nothing about the design.
